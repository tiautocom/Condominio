    ...
    public class UsuarioRegraNegocios
    {
        ...
        public string Salvar(Usuarios usuario)
        {
            try
            {
                ...
                conexaoSqlServer.AdicionarParametros("@IdTipoUsuario", 3);
        ...
        public DataTable Logar(string tel, string senha)
        {
            try
            {
                conexaoSqlServer.LimparParametros();
                conexaoSqlServer.AdicionarParametros("@Telefone", tel);
                conexaoSqlServer.AdicionarParametros("@Senha", senha);

                DataTable dadosTabela = new DataTable();
                dadosTabela = conexaoSqlServer.ExecutarConsulta(CommandType.StoredProcedure, "uspLoginUsuario");
                return dadosTabela;
            }
            catch (Exception ex)
            {
